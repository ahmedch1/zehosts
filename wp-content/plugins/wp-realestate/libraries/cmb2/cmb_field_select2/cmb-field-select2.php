...
class PW_CMB2_Field_Select2 {
	...
	public function get_pw_multiselect_options( $field_escaped_value = array(), $field_type_object ) {
		...
		if ( ! empty( $field_escaped_value ) && is_array($field_escaped_value) ) {
	...
	public function setup_admin_scripts() {
		...
		wp_enqueue_script( 'pw-select2-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'wre-select2', 'jquery-ui-sortable' ), self::VERSION );
		//wp_register_style( 'pw-select2', $asset_path . '/css/select2.min.css', array(), '4.0.3' );
		wp_enqueue_style( 'pw-select2-tweaks', $asset_path . '/css/style.css', array( 'wre-select2' ), self::VERSION );
	}
}
